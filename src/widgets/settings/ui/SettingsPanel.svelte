<script lang="ts">
    ...
    const hasCodeThemeSettings = $derived(
        supportsCodeThemeSettings(page.route?.id),
    );
    const showBackgroundModeList = $derived(
        !readingMode.value && BackgroundModeList,
    );
    ...
    function onPanelKeyDown(e: KeyboardEvent) {
        if (e.key === "Tab") {
            ...
            const isFirstActive =
                first.contains(document.activeElement) || e.target === first;
            const isLastActive =
                last.contains(document.activeElement) || e.target === last;
    ...
    {#if isOpen}
        ...
        >
            ...
            {#if showBackgroundModeList}
                <hr
                    data-testid="settings-divider"
                    class="border-0 border-t border-[--color-border] my-1"
                />
                <BackgroundModeList />
            {/if}
